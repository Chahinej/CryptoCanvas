...
\begin{figure}[ht!]
\centering
\frame{\includegraphics[width=35em, height=137mm]{figures/UML Design/Use Case Diagram.png}}
...
\begin{figure}[ht!]
\centering
\frame{\includegraphics[width=35em, height=160mm]{figures/UML Design/Sequence Diagram.png}}
...
\begin{figure}[ht!]
\centering
\frame{\includegraphics[width=35em, height=160mm]{figures/UML Design/Activity Diagram.png}}
...
\begin{figure}[ht!]
\centering
\frame{\includegraphics[width=35em, height=160mm]{figures/UML Design/Class Diagram.png}}
...
\begin{figure}[ht!]
\centering
\frame{\includegraphics[width=37em, height=210mm]{figures/UML Design/Code snippet.png}}
...
\end{center}
\chapter{Tools And Development Phases}
\section{Programming Language}
\begin{itemize}
\item Python was chosen for implementing SSIS due to its simplicity, readability, and availability of relevant libraries.
\item Python's easy-to-understand syntax and emphasis on code readability simplify the implementation of complex algorithms like SSIS.
\end{itemize}

\section{Development Environment}
\begin{itemize}
\item \textbf{Visual Studio Code (VS Code):} Lightweight code editor with Python support through extensions. Provides syntax highlighting, debugging, and Git integration. Customizable and widely used.

\item \textbf{Jupyter Notebook:} Interactive computing environment for creating and sharing code, visualizations, and text. Ideal for data analysis and documentation. Supports multiple languages, including Python.
\end{itemize}
\vskip 5em
...
\section{Additional Software or Hardware Tools}
\begin{enumerate}
...
\subsection{Functions}
 \vskip2em
\begin{enumerate}
    \item \textbf{Function 1: Key Validation}
    \begin{itemize}
        \item This function will check the format and validity of a given key.
        \item It will take a key as input.
        \item The function will verify if the key meets the required format criteria.
        \item If the key is invalid, the program will terminate.
    \end{itemize}
     \vskip2em
    \item \textbf{Function 2: Matrix Interleaving}
    \begin{itemize}
        \item This function will perform the reorganization of a matrix based on a given key.
        \item The function will iterate through each row of the matrix.
        \item For each row, it will iterate through the elements of the key
        \item The value from the source matrix corresponding to the key element will be assigned to the destination matrix.
    \end{itemize}
    \vskip2em
    \item \textbf{Function 3: Noise Generation}
    \begin{itemize}
        \item This function will generate Additive White Gaussian Noise (AWGN) \cite{enwiki:1154048521} for a given image length.
        \item It will take the image length as input.
        \item The function will use appropriate libraries to generate a sequence of random numbers following a Gaussian distribution.
        \item The sequence will be reshaped into a matrix format based on the image length.
        \item The normalized noise matrix will be returned.
    \end{itemize}
    \vskip6em
    \item \textbf{Function 4: Second Stage Noise Generation}
    \begin{itemize}
...
\textbf{The decoding process involves  these  steps: \textcolor{red}{(Reversing Encoding Steps)}}
\begin{enumerate}
    \item Reading in the stego image using the OpenCV library and extracting the binary representation of the blue color channel of each pixel.
    \item Reconstructing the interleaved pixels using the secret key.
    \item Recovering the noise signal matrix using the pseudorandom number generator.
    \item Dividing the binary representation of each pixel by the corresponding value from the noise signal matrix to restore the original binary representation.
    \item Extracting the binary message from the LSBs of the blue color channel using the secret key.
\end{enumerate}
...
\begin{figure}[ht!]
\centering
\frame{\includegraphics[width=37.5em, height=130mm]{figures/Summary.png}}
\caption{Tools Used}
\end{figure}